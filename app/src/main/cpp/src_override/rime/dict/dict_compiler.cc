...
bool DictCompiler::Compile(const path& schema_file) {
  ...
  if (build_table_from_source && dict_files.empty()) {
    LOG(WARNING) << "no dictionary source files for '" << dict_name_
  ...
  if (build_table_from_source) {
    ...
    if (prism_->Exists() && prism_->Load()) {
      LOG(INFO) << "reuse existing prism: " << prism_->file_path();
      rebuild_prism = false;
      prism_->Close();
    } else {
      rebuild_prism = true;
    ...
    if (!std::filesystem::exists(dict_file)) {
      if (pack_table->Exists())
        LOG(INFO) << "pack source file '" << dict_file
                  << "' does not exist; reuse existing table: "
                  << pack_table->file_path();
      else
        LOG(WARNING) << "neither " << pack_name << ".dict.yaml nor "
                     << pack_name << ".table.bin exists.";
      continue;
    }
