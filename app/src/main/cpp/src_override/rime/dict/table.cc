...
bool Table::Query(const SyllableGraph& syll_graph,
  ...
  while (!q.empty()) {
    ...
    bool is_fuzzy = q.front().is_fuzzy;
    q.pop();
    ...
    if (query.level() == Code::kIndexCodeMaxLength) {
      ...
      accessor.set_is_fuzzy(is_fuzzy);
    ...
    for (const auto& spellings : index->second) {
      ...
      for (auto props : spellings.second) {
        ...
        // Supplemental lookup never consumes partial-word candidates. Avoid
        // fetching and collecting them, but keep traversing their prefixes.
        if (!full_words_only || end_pos == syll_graph.input_length) {
          TableAccessor accessor =
              query.Access(syll_id, next_credibility, delta_quality_len);
          accessor.set_is_fuzzy(is_fuzzy || props->type == kFuzzySpelling);
          if (!accessor.exhausted()) {
        ...
        if (end_pos < syll_graph.interpreted_length &&
            query.Advance(syll_id, next_credibility, delta_quality_len,
                          current_pos)) {
          q.push({end_pos, query, is_fuzzy || props->type == kFuzzySpelling});
          query.Backdate();
        }
      }
    }
