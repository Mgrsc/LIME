...
struct DfsState {
  ...
  vector<bool> is_fuzzy;
  hash_map<int, DictEntryList> query_result;
  map<pair<size_t, string>, an<DictEntry>> recruited;
  map<pair<size_t, Code>, std::tuple<bool, double, double>> visited;
...
void DfsState::RecruitEntry(size_t pos,
  ...
  if (e) {
    ...
    auto inserted = recruited.emplace(std::make_pair(pos, key), e);
    if (inserted.second) {
      query_result[pos].push_back(e);
    } else {
      auto& best = inserted.first->second;
      if ((e->IsExactMatch() && !best->IsExactMatch()) ||
          (e->IsExactMatch() == best->IsExactMatch() &&
           ((!e->is_fuzzy && best->is_fuzzy) ||
            (e->is_fuzzy == best->is_fuzzy &&
             (e->weight > best->weight ||
              (e->weight == best->weight &&
               e->quality_len > best->quality_len)))))) {
        *best = *e;
      }
    }
  }
...
// Traverse dictionary prefixes in syllable order. Each alternative spelling
// seeks the same prefix again: LevelDbAccessor::Jump and TextDbAccessor::Jump
// both support backward seeks, including after an exhausted recursive scan.
void UserDictionary::DfsLookup(const SyllableGraph& syll_graph,
  ...
  // Equivalent prefixes share the same remaining search. Revisit only when a
  // better spelling path can improve the result, rather than expanding every
  // combination of overlapping abbreviations.
  auto score =
      std::make_tuple(!state->is_fuzzy.back(), state->credibility.back(),
                      state->quality_len.back());
  auto visited =
      state->visited.emplace(std::make_pair(current_pos, state->code), score);
  if (!visited.second) {
    if (visited.first->second >= score)
      return;
    visited.first->second = score;
  }
  ...
  for (const auto& spelling : index->second) {
    ...
    for (size_t i = 0; i < spelling.second.size(); ++i) {
      ...
      state->is_fuzzy.push_back(state->is_fuzzy.back() ||
                                props->type == kFuzzySpelling);
      state->credibility.push_back(state->credibility.back() +
                                   props->credibility);
      size_t end_pos = props->end_pos;
      // Quality length score for full-code matches
...
an<UserDictEntryCollector> UserDictionary::Lookup(
  ...
  state.is_fuzzy.push_back(false);
  state.accessor = db_->Query("");
  if (!state.ForwardScan(" "))  // skip metadata; no words to search
    return nullptr;
  string prefix;
