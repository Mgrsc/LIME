...
struct DictEntry {
  ...
  double weight = 0.0;
  // Quality length score for full-code matches: total character length of "full pinyin" segments along the entry path
  double quality_len = 0.0;
  // Query-path metadata only; never serialized to the table or user database.
  bool is_fuzzy = false;
  int commit_count = 0;
