...
CodeMatch match_extra_code(const table::Code* extra_code,
  ...
  for (const SpellingProperties* props : spellings->second) {
    ...
    if (!match.success)
      continue;
    match.credibility += props->credibility;
    if (props->type == kNormalSpelling)
      match.quality_len += props->end_pos - current_pos;
    match.is_fuzzy |= props->type == kFuzzySpelling;
    if (!best_match.success || match.end_pos > best_match.end_pos ||
        (match.end_pos == best_match.end_pos &&
         ((!match.is_fuzzy && best_match.is_fuzzy) ||
          (match.is_fuzzy == best_match.is_fuzzy &&
           match.credibility > best_match.credibility))))
      best_match = match;
  }
  return best_match;
}
...
an<DictEntry> DictEntryIterator::Peek() {
  if (!entry_ && !exhausted()) {
    ...
    entry_->is_fuzzy = chunk.is_fuzzy;
...
static void lookup_table(Table* table,
                         ...
                         bool full_words_only) {
  TableQueryResult result;
  if (!table->Query(syllable_graph, start_pos, &result, full_words_only)) {
    return;
  }
  // A supplemental query needs complete words only. Bound the remaining
  // syllable count before walking each long entry's extra code.
  vector<pair<size_t, size_t>> remaining;
  if (full_words_only) {
    remaining.resize(syllable_graph.input_length + 1,
                     {syllable_graph.input_length + 1, 0});
    remaining.back() = {0, 0};
    for (auto start = syllable_graph.edges.rbegin();
         start != syllable_graph.edges.rend(); ++start) {
      for (const auto& end : start->second) {
        auto suffix = remaining[end.first];
        if (suffix.first > syllable_graph.input_length)
          continue;
        auto& bounds = remaining[start->first];
        bounds.first = std::min(bounds.first, suffix.first + 1);
        bounds.second = std::max(bounds.second, suffix.second + 1);
      }
    }
  }
  // copy result
  for (auto& v : result) {
    ...
    for (TableAccessor& a : v.second) {
      ...
      if (a.extra_code()) {
        do {
          if (full_words_only &&
              (a.extra_code()->size < remaining[end_pos].first ||
               a.extra_code()->size > remaining[end_pos].second))
            continue;
          dictionary::CodeMatch match = dictionary::match_extra_code(
              a.extra_code(), 0, syllable_graph, end_pos, predict_word);
          if (!match.success ||
              (full_words_only && match.end_pos != syllable_graph.input_length))
            continue;
          size_t matching_code_size = a.index_code().size() + match.depth;
          (*collector)[match.end_pos].AddChunk(
              {table, a.code(), a.entry(), matching_code_size,
               cr + match.credibility, q + match.quality_len,
               a.is_fuzzy() || match.is_fuzzy});
        } while (a.Next());
      } else if (!full_words_only || end_pos == syllable_graph.input_length) {
...
an<DictEntryCollector> Dictionary::Lookup(const SyllableGraph& syllable_graph,
  ...
  for (const auto& table : tables_) {
    ...
    lookup_table(table.get(), collector.get(), syllable_graph, start_pos,
                 predict_word, initial_credibility, full_words_only);
  }
  if (collector->empty())
    return nullptr;
