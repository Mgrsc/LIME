...
// Store is_correction in higher bits of SpellingDescriptor::type, avoiding sign bit
...
bool Prism::Load() {
  ...
  // Version check: compatible with both 3.0 and 4.0
  if (format_ < 3.0 - DBL_EPSILON) {
  ...
  if (script) {
    ...
    for (; i != script->end(); ++i, ++item) {
      ...
      for (; j != i->second.end(); ++j, ++desc) {
        ...
        // Pack and write type + is_correction
