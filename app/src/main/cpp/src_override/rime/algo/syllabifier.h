...
struct EdgeProperties : SpellingProperties {
  ...
  // Starting positions of ambiguous segmentation encoding segments
...
struct SyllableGraph {
  bool pruned_abbreviations = false;
...
class Syllabifier {
 public:
  ...
  RIME_DLL int BuildSyllableGraph(const string& input,
                                  Prism& prism,
                                  SyllableGraph* graph,
                                  SyllableGraph* abbreviated_graph = nullptr);
  RIME_DLL void EnableCorrection(Corrector* corrector);

 protected:
  void PruneGraph(SyllableGraph* graph,
                  size_t farthest,
