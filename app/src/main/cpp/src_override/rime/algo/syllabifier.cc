...
// Penalty hierarchy:
// 1. Full pinyin: user typed all spellings. Penalty = 0
// 2. Abbreviation: user typed initials explicitly to match characters. Penalty ≈ -2.3
// 3. Completion: incomplete syllable completed by algorithm. Penalty ≈ -3.0
...
int Syllabifier::BuildSyllableGraph(const string& input,
                                    Prism& prism,
                                    SyllableGraph* graph,
                                    SyllableGraph* abbreviated_graph) {
  if (!graph)
    return 0;
  *graph = SyllableGraph();
  if (abbreviated_graph)
    *abbreviated_graph = SyllableGraph();
  if (input.empty())
    return 0;
      ...
      for (const auto& m : matches) {
        ...
        while (!accessor.exhausted()) {
          ...
          if (strict_spelling_ && matches_input &&
            ...
            has_abbreviation |= props.type == kAbbreviation;
  ...
  // Capture the unpruned graph only when abbreviation suppression is possible.
  // Both lookups share this prism traversal; no second spelling search is
  // needed.
  if (abbreviated_graph && has_abbreviation &&
      graph->vertices[farthest] < kAbbreviation) {
    abbreviated_graph->vertices = graph->vertices;
    abbreviated_graph->edges = graph->edges;
  }
  PruneGraph(graph, farthest, false);
  if (abbreviated_graph && graph->pruned_abbreviations) {
    // Supplemental words must cover the input without completion or correction.
    PruneGraph(abbreviated_graph, farthest, true);
    abbreviated_graph->input_length = input.length();
    abbreviated_graph->interpreted_length = farthest;
    Transpose(abbreviated_graph);
  } else if (abbreviated_graph) {
    *abbreviated_graph = SyllableGraph();
  }
...
void Syllabifier::PruneGraph(SyllableGraph* graph,
                             size_t farthest,
                             bool preserve_abbreviations) {
    ...
    for (auto j = graph->edges[i].begin(); j != graph->edges[i].end();) {
      ...
      for (auto k = j->second.begin(); k != j->second.end();) {
        ...
        if (k->second.type > last_type) {
          if (k->second.type == kAbbreviation)
            graph->pruned_abbreviations = true;
          j->second.erase(k++);
        } else {
      ...
      if (j->second.empty()) {
        ...
        if (!preserve_abbreviations && edge_type < kAbbreviation)
    ...
    for (auto& x : x_end_vertices) {
      ...
      if (x.first == end) {
        ...
        for (auto& spelling : x.second) {
          // This edge (X) forms an ambiguity with respect to start position
